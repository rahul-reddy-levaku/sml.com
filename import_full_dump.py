...
def import_vouchers_and_postings():
    ...
    def next_auto_vno():
        nonlocal auto_idx
        # Keep it ≤ 20 chars (Voucher.voucher_no max_length=20)
        while True:
            v = f"VCH{auto_idx:07d}"  # VCH0000001 … fits in 10 chars
            auto_idx += 1
            # ensure uniqueness across DB too
    ...
    for row in rdr:
        cnt += 1
        try:
            vno_raw = ci_get(row, "VoucherNo", "VNo", "Voucher")
            ...
            if not ah:
                # make a placeholder account head to avoid FK/NOT NULL issues
                ah = ensure_account_head("AHAUTO", name="Auto Account")
